...
# import sys
# sys.path.append('/cosma5/data/dp004/dc-love2/sph2sed')
...
# sp.tag = tag
# sp.redshift = float(sp.tag[5:].replace('p','.'))
# sp.scale_factor = 1.0 / (1+sp.redshift)

p_sgrpn = E.readArray('PARTDATA', directory, tag, '/PartType4/SubGroupNumber')
p_grpn = E.readArray('PARTDATA', directory, tag, '/PartType4/GroupNumber')
p_imass = E.readArray('PARTDATA', directory, tag, '/PartType4/InitialMass') * 1e10
p_form = E.readArray('PARTDATA', directory, tag, '/PartType4/StellarFormationTime')
p_metal = E.readArray('PARTDATA', directory, tag, '/PartType4/SmoothedMetallicity')
p_cood = E.readArray('PARTDATA', directory, tag, '/PartType4/Coordinates', physicalUnits=True)

shgrpno = E.readArray('SUBFIND', directory, tag, '/Subhalo/SubGroupNumber')
grpno = E.readArray('SUBFIND', directory, tag, '/Subhalo/GroupNumber')
mstar_30 = E.readArray('SUBFIND', directory, tag, '/Subhalo/ApertureMeasurements/Mass/030kpc')[:,4] * 1e10
mstar = E.readArray('SUBFIND', directory, tag, '/Subhalo/Stars/Mass') * 1e10
sf_metal = E.readArray('SUBFIND', directory, tag, '/Subhalo/SF/Metallicity')
sf_mass = E.readArray('SUBFIND', directory, tag, '/Subhalo/SF/Mass') * 1e10
sfr_30 = E.readArray('SUBFIND', directory, tag, '/Subhalo/ApertureMeasurements/SFR/030kpc')
sfr = E.readArray('SUBFIND', directory, tag, '/Subhalo/StarFormationRate')
cop = E.readArray('SUBFIND', directory, tag, '/Subhalo/CentreOfPotential', physicalUnits=True)

log_masses = np.log10(mstar)

# indices = np.where((log_masses > 10.5) & (log_masses < 10.8))[0]
indices = np.where(log_masses > 10.5)[0]
...
with h5py.File('data/full_histories_eagle.h5','a') as f:
    ...
    for key in indices:
        ...
        # f["Subhalos/Stellar Metallicity"][i] = (stellar_metallicity * 1e10) / cosmo.h
        f["Subhalos/SFR"][i] = sfr[key]
        f["Subhalos/Star Forming Gas Mass"][i] = sf_mass[key]
        f["Subhalos/Stellar Mass 30kpc"][i] = mstar_30[key] 
        f["Subhalos/Stellar Metallicity 30kpc"][i] = np.mean(p_metal[mask])
...
# sp.filename = '/gpfs/data/dc-love2/full_histories_eagle_test.p'
# sp.save()
